...
if [ $# -lt 1 ]; then
    ...
    echo "TESTER_NAME must be one of ['symcc', 'symsan', 'aflplusplus', 'concolic']"
    exit 1
fi

if [ $# -eq 2 ]; then
    TIMEOUT_SECONDS=$2
    echo "Using specified timeout of $TIMEOUT_SECONDS for testing woff2 with $TESTER_NAME."
else
    TIMEOUT_SECONDS=60
    echo "Using default timeout of 60 seconds for testing woff2 with $TESTER_NAME."
fi
...
export GCOV_ROOT="/woff2-gcov"
export KLEE_ROOT="/woff2-klee"
export SYMCC_ROOT="/woff2-symcc"
export SYMSAN_ROOT="/woff2-symsan"
export AFLPLUSPLUS_ROOT="/woff2-aflplusplus"
export INSTR_ROOT="/woff2-instr"
export BIN="woff2_decompress"

export TESTER_EXECUTE_ARGS="@@"    # no arguments and stdin
export GCOVE_EXECUTE_ARGS=""
export KLEE_SYM_ARGS="A --sym-files 1 584"
...
case $TESTER_NAME in
  ...
  "aflplusplus")
    ...
    timeout -k 10 $TIMEOUT_SECONDS /bin/bash -c \
        "afl-fuzz -i $INPUT -o $OUTPUT ${AFLPLUSPLUS_ROOT}/${BIN} -m none ${TESTER_EXECUTE_ARGS}" \
...
#     output=$(gcov -r -b -o $src_dir "$basename" 2>&1)
